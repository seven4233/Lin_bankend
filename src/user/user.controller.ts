...
export class UserController {
    ...
    // 添加搜索历史
    @Post('history')
    history(@Body('keyword') keyword, @Req() req){
        const userId = req.currentUser?.id
        return this.userService.addHistory(keyword, userId)
    }

    //获取搜索历史
    @Get('history')
    queryHistory(@Req() req){
        const userId = req.currentUser?.id
    ...
    //删除搜索历史
    @Delete('history')
    deleteHistory(@Query("id") id){
        return this.userService.deleteHistory(id)
    }
